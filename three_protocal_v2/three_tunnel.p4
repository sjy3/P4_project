...
control MyIngress(inout headers hdr,
    ...
    action mytunnel_forward(ip4Addr_t src, ip4Addr_t dst, macAddr_t dstAddr, egressSpec_t port) {
        standard_metadata.egress_spec = port;
        hdr.ethernet.srcAddr = hdr.ethernet.dstAddr;
        hdr.ethernet.dstAddr = dstAddr;
        hdr.ipv4_tunnel.ttl = hdr.ipv4.ttl - 1;
	hdr.ipv4_tunnel.srcAddr = src;
	hdr.ipv4_tunnel.dstAddr = dst;	
...
//          hdr.ipv4_tunnel.dstAddr: lpm;
    ...
    action ipv6_forward(ip6Addr_t src, ip6Addr_t dst, macAddr_t dstAddr, egressSpec_t port) {
        standard_metadata.egress_spec = port;
        hdr.ethernet.srcAddr = hdr.ethernet.dstAddr;
        hdr.ethernet.dstAddr = dstAddr;
        hdr.ipv6.hoplimit = hdr.ipv4.ttl - 1;
	hdr.ipv6.dstAddr = src;
	hdr.ipv6.scrAddr = dst;
    }
...
//          hdr.ipv6.dstAddr: lpm;
    ...
    action sinet_forward(sinetAddr_t src, sinetAddr_t dst, macAddr_t dst_mac_addr, egressSpec_t port) {
        standard_metadata.egress_spec = port;
        hdr.ethernet.srcAddr = hdr.ethernet.dstAddr;
        hdr.ethernet.dstAddr = dst_mac_addr;
        hdr.sinet.hop_limit = hdr.ipv4.ttl - 1;
        meta.sinetAddr_src = src;
        meta.sinetAddr_dst = dst;
